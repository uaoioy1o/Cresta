...
testset2=torchvision.datasets.ImageFolder(curr_dir3,transform=transform_test)
testloader2 = torch.utils.data.DataLoader(testset2, batch_size=BATCH_SIZE, shuffle=True, num_workers=2)
        ...
        with open("log.txt", "w")as f2:
            for epoch in range(pre_epoch, EPOCH):
                ...
                with torch.no_grad():   #里边的数据不需要计算梯度，不需要进行反向传播
                    ...
                    f.write("EPOCH=%03d,Accuracy= %.3f%%,Extraction= %.3f%%" % (epoch + 1, acc, acc2))
                    f.write('\n')
                    f.flush()
                    # 记录最佳测试分类准确率并写入best_acc.txt文件中
                    if acc > best_acc:
                        f3 = open("best_acc.txt", "w")
                        f3.write("EPOCH=%d,best_acc= %.3f%%,ext_rate= %.3f%%" % (epoch + 1, acc, acc2))
                        f3.close()
                        best_acc = acc
